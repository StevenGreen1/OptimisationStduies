...
class Results:
    ...
    m_HadronicEnergyTrunc = {0:1000000, 38:1, 69:0.5, 70:0.75, 71:1, 72:1.5, 73:2, 74:5, 75:10, 76:1000000, 77:0.5, 78:0.75, 79:1, 80:1.5, 81:2, 82:5, 83:10, 84:1000000}
    ...
    m_DetectorLabel = {38:'High Energy Jets', 45:'Steel HCal Absorber Material, QGSP_BERT', 46:'Steel HCal Absorber Material, QGSP_BERT_HP', 47:'Tungsten HCal Absorber Material, QGSP_BERT', 48:'Tungsten HCal Absorber Material, QGSP_BERT_HP'}
...
#==================== 
    ...
    def __init__(self, resultsName, detectorModelList, xAxisPlottingList, xAxisTitle, reconstructionVariantList, jetEnergyList, algorithm, fixedRecoVar):
        ...
        self.xAxisPlottingList = xAxisPlottingList
        self.minXAxis = 0 #xAxisPlottingList[0] - (xAxisPlottingList[1] - xAxisPlottingList[0])
        self.maxXAxis = 1 #xAxisPlottingList[-1] + (xAxisPlottingList[-2] - xAxisPlottingList[-3])
...
#==================== 
    ...
    def readData(self):
        ...
        fileName = '/var/clus/usera/sg568/ilcsoft_v01_17_05/HCAL_Optimisation_Studies/AnalysePerformanceResults/RecoVar4/Results_GJN38.txt'
        reconstructionVariant = 0
        detectorModel = 0
        jetEnergy = 0
        pandoraSettings = ''
        if os.path.isfile(fileName):
            ...
            for line in allLines:
                 print line
                 regex = re.compile("Job Number (\d+)")
                 searchResults = regex.search(line.strip())
                 if searchResults is not None:
                     detectorModel = int(searchResults.group(1))

                 regex = re.compile("Pandora Settings(.*?)")
                 searchResults = regex.search(line.strip())
                 if searchResults is not None:
                     print 'Pandora settings match'
                     pandoraSettings = line.strip()[17:]
                     print pandoraSettings
                     if 'PerfectPhotonNANK0LAK0L' in pandoraSettings:
                         pandoraSettings = 'PerfectPhotonNK0L'

                 regex = re.compile("Jet Energy (\d+)")
                 searchResults = regex.search(line.strip())
                 if searchResults is not None:
                     jetEnergy = float(searchResults.group(1))
