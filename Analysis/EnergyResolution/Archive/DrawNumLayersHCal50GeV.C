...
void DrawNumLayersHCal50GeV() 
{
    ...
    TH2F *pAxes = new TH2F("axesEj","",100,16,62,1000,10,20);
    pAxes->SetTitle("50 GeV Kaon0L Energy Resolution vs Number of Layers in HCal");
    pAxes->GetYaxis()->SetTitle("#sigma_{Reco} / E_{Reco}");
    pAxes->GetXaxis()->SetTitle("Number of HCal Layers");
    pAxes->Draw();

    pTGraphErrors->Draw("same PL");
    const std::string name("HCal" + NumberToString(energy) + "GeVKaon0LResVsLayerNumber.C");
