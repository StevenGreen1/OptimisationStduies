...
void Draw_ER_vs_NHCalVariableLayers() 
{
    ...
    detectorModels.push_back(49);
    detectorModels.push_back(50);
    ...
    detModelToLayerNumber[49] = 18;
    detModelToLayerNumber[50] = 24;
    ...
    TH2F *pAxes = new TH2F("axesEj","",100,14,64,1000,0,20);
    pAxes->SetTitle("50 GeV Kaon0L Energy Resolution vs HCal Number Variable Layers");
    pAxes->GetYaxis()->SetTitle("#sigma_{Reco} / E_{Reco}");
    pAxes->GetXaxis()->SetTitle("NHCal Variable Layers");
    pAxes->Draw();

    pTGraphErrors->Draw("same PL");
    const std::string name("ER_vs_NHCalVariableLayers_" + NumberToString(energy) + "GeVKaon0L.C");
    const std::string name2("ER_vs_NHCalVariableLayers_" + NumberToString(energy) + "GeVKaon0L.pdf");
