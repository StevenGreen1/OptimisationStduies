...
void Draw_ER_vs_ScECalNLayers() 
{
    ...
    TH2F *pAxes = new TH2F("axesEj","",100,14,32,1000,2.4,3.8);
    pAxes->SetTitle("100 GeV Photon Energy Resolution vs Number of Layers in ECal (Sc)");
    pAxes->GetYaxis()->SetTitle("#sigma_{Reco} / E_{Reco}");
    pAxes->GetXaxis()->SetTitle("Number of ECal Layers");
    pAxes->Draw();

    pTGraphErrors->Draw("same PL");
    const std::string name("ER_vs_ScECalNLayers_" + NumberToString(energy) + "GeVPhoton.C");
    const std::string name2("ER_vs_ScECalNLayers_" + NumberToString(energy) + "GeVPhoton.pdf");
