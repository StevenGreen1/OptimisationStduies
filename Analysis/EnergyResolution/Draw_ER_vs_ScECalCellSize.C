...
void Draw_ER_vs_ScECalCellSize() 
{
    const int recoVar(71);
    const int energy(100);

    std::vector<int> detectorModels;
    detectorModels.push_back(90);
    detectorModels.push_back(91);
    detectorModels.push_back(92);
    detectorModels.push_back(93);
    detectorModels.push_back(94);
    detectorModels.push_back(95);

    std::map<int, int> detModelToLayerNumber;
    detModelToLayerNumber[90] = 3;
    detModelToLayerNumber[91] = 5;
    detModelToLayerNumber[92] = 7;
    detModelToLayerNumber[93] = 10;
    detModelToLayerNumber[94] = 15;
    detModelToLayerNumber[95] = 20;
    ...
    TH2F *pAxes = new TH2F("axesEj","",100,0,25,1000,0,5);
    pAxes->SetTitle("100 GeV Photon Energy Resolution vs Cell Size in ECal (Sc)");
    pAxes->GetYaxis()->SetTitle("#sigma_{Reco} / E_{Reco} [%]");
    pAxes->GetXaxis()->SetTitle("ECal Cell Size [mm]");
    pAxes->Draw();

    pTGraphErrors->Draw("same PL");
    const std::string name("ER_vs_ScECalCellSize_" + NumberToString(energy) + "GeVPhoton.C");
    const std::string name2("ER_vs_ScECalCellSize_" + NumberToString(energy) + "GeVPhoton.pdf");
