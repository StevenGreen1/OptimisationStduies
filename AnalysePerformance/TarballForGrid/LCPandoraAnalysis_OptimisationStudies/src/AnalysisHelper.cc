...
void AnalysisHelper::CalculatePerformance(const TH1F *const pTH1F, float &resolution, float &resolutionError, bool fixDistributionCentre, bool print, const std::string &outputFileName)
{
    ...
    if (5 > pTH1F->GetEntries())
    {
        ...
        resultsFile << pTH1F->GetName() << " (" << pTH1F->GetEntries() << " entries) - skipped" << std::endl;
        resultsFile.close();
        return;
    }
    ...
    if (print)
    {
        std::ofstream resultsFile;
        resultsFile.open (outputFileName.c_str(), std::ios_base::app);
        resultsFile << pTH1F->GetName() << " (" << pTH1F->GetEntries() << " entries), rawrms: " << rawRms << ", rms90: " << rmsmin
                  << " (" << low << "-" << high << "), mean: " << mean << ", sigma: " << sigma << "+-" << sigmasigma;
        (fixDistributionCentre) ? (resultsFile << ", sE/E: " << frac << "+-" << efrac << std::endl) : (resultsFile << std::endl);
        resultsFile.close();
    }
