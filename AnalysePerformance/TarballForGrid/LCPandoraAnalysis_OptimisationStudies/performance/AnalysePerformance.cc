...
void AnalysePerformance(TChain *pTChain, const std::string &outputRootFileName, const std::string &outputFileName);
    ...
    try
    {
        ...
        if ((nArgs < 3) || (nArgs > 3))
        {
            std::cout << std::endl
                      ...
                      << "  inputRunFile   : file containing list of root files to add" << std::endl
                      << "  outputFileName : output root file, for histogram output" << std::endl 
                      << "  outputTextFile : output text file, for numerical output" << std::endl << std::endl;
            return 1;
        }

        const std::string inputRunFile(argv[1]);
        const std::string outputRootFileName(argv[2]);
        const std::string outputFileName(argv[3]);

        std::ifstream runfile(inputRunFile.c_str());
        std::string lineInRunFile;

        TChain *pTChain = new TChain("PfoAnalysisTree");

        while(runfile >> lineInRunFile) 
        {
            pTChain->Add(lineInRunFile.c_str());
        }
        ...
        AnalysePerformance(pTChain, outputRootFileName, outputFileName);
        delete pTChain;
    }
...
void AnalysePerformance(TChain *pTChain, const std::string &outputRootFileName, const std::string &outputFileName)
{
    ...
    AnalysisHelper::CalculatePerformance(pPFAL7A, resolution, resolutionError, true, true, outputFileName);

    for (unsigned int i = 0; i < nRegionBins; ++i)
    {
        resolution = 0.f; resolutionError = 0.f;
        AnalysisHelper::CalculatePerformance(pRegionHistograms[i], resolution, resolutionError, true, true, outputFileName);
    ...
    if (!outputRootFileName.empty())
    {
        ...
        resultsFile.open (outputFileName.c_str(), std::ios_base::app);
        resultsFile << "Will write histograms to file : " << outputRootFileName << std::endl;
        resultsFile.close();
